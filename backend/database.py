...
# Only relevant for SQLite — Postgres enforces FKs by default.
# ---------------------------------------------------------------------------
if _is_sqlite:
...
def _run_migrations(bind):
    ...
    NOTE: These migrations use SQLite-specific syntax (PRAGMA, ALTER TABLE
    workarounds). On PostgreSQL, create_all() should handle the schema
    correctly from scratch, so we skip these migrations.
    """
    if not _is_sqlite:
        # On PostgreSQL, create_all() handles the full schema.
        # Skip SQLite-specific migration hacks.
        return
